...
import { Badge, Card, Empty, ErrorNote, LabelBadge, Spinner, Stat } from "../components/primitives";

/** Scene 3: the result is inspectable. Evidence, retrieval, blast radius, attack chain. */
export function Incident({ incidentId }: { incidentId: string }) {
  ...
  if (loading) return <Card><Spinner label="Loading incident…" /></Card>;
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      >
        ...
        {detail.summary && (
          <pre className="mt-3 max-h-48 overflow-auto rounded border border-ink-800 bg-ink-950 p-3 text-xs leading-relaxed whitespace-pre-wrap text-ink-300">
            {detail.summary}
          </pre>
        )}
        ...
        <div className="mt-3 flex flex-wrap gap-1.5">
          ...
          {detail.mitre_techniques
            ...
            .slice(0, 8)
            .map((t) => (
              <Badge key={t} className="border-accent/30 bg-accent/10 text-accent">
                {t}
              </Badge>
            ))}
        </div>
      ...
      >
        ...
        <ul className="space-y-2">
          {similar.map((hit) => (
            <li key={hit.incident_id} className="rounded border border-ink-800 bg-ink-850 p-2.5">
              <div className="flex items-center justify-between gap-2">
                <span className="mono text-xs text-ink-200">{hit.incident_id}</span>
                <div className="flex items-center gap-2">
                  ...
                  <span className="mono text-xs text-ink-400">{hit.score.toFixed(2)}</span>
                </div>
              </div>
              <p className="mt-1 text-[11px] text-ink-400">{hit.why}</p>
            </li>
          ))}
        </ul>
        ...
        {!graph || graph.node_count === 0 ? (
          ...
          <>
            ...
            {graph.blast_radius.hubs_blocked.length > 0 && (
              <p className="mt-2 rounded border border-bp/30 bg-bp/10 px-2 py-1.5 text-[11px] text-bp">
            ...
            <div className="mt-3 space-y-1.5">
              {Object.entries(graph.blast_radius.impacted_by_type).map(([type, values]) => (
                <div key={type} className="flex gap-2 text-xs">
                  <span className="w-16 shrink-0 text-ink-400">{type}</span>
                  <span className="mono truncate text-ink-300" title={values.join(", ")}>
            ...
            {graph.attack_path && graph.attack_path.hops > 0 && (
              <div className="mt-3 rounded border border-ink-800 bg-ink-850 p-2.5">
                ...
                <div className="mono mt-1.5 flex flex-wrap items-center gap-1 text-[11px] text-ink-300">
                  {graph.attack_path.path.map((node, i) => (
                    <span key={`${node}-${i}`} className="flex items-center gap-1">
                      <span className="rounded bg-ink-800 px-1.5 py-0.5">{node}</span>
      ...
      >
        <div className="-mx-4 -mb-4 max-h-80 overflow-auto">
          <table className="w-full min-w-[720px] text-xs">
            ...
            <tbody>
              {evidence.map((row) => (
                <tr key={row.evidence_id} className="border-b border-ink-850">
                  <td className="mono px-4 py-1.5 text-ink-300">{row.evidence_id}</td>
                  <td className="px-2 py-1.5 text-ink-300">{row.entity_type || "—"}</td>
                  <td className="px-2 py-1.5 text-ink-400">{row.evidence_role || "—"}</td>
                  ...
                  <td className="mono px-4 py-1.5 text-ink-400">
